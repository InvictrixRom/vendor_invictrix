...
func init() {
	android.RegisterModuleType("invictrix_generator", GeneratorFactory)
...
func (g *Module) GenerateAndroidBuildActions(ctx android.ModuleContext) {
	...
	if depRoot == "" {
		...
		depRoot = invictrixExpandVariables(ctx, depRoot)
	}

	// Glob dep_files property
	for _, dep_file := range g.properties.Dep_files {
		dep_file = invictrixExpandVariables(ctx, dep_file)
	...
	cmd := invictrixExpandVariables(ctx, String(g.properties.Cmd))
